...
The initial automated test plan for this QAS was to run a test script in the CI pipeline that utilized selenium. This automated testing would pass valid and invalid info into the form on the app's input pages to determine if any invalid input would be passed to MongoDB. However it was discovered that this is more difficult than initially estimated since the app does not return any HTTP request when invalid input is passed to it. Instead, the HTML UI prevents the user from submitting bad data on the client side of the app, so the POST request with invalid input is never made. Due to this, it was determined, automated testing for this QAS was not neccesary, and manual testing was sufficient. The HTML input validation process is described below. 

###All input validation is done using the HTML form that takes purchase data from the user. 
- The category is a simple drop-down menu with 5 options including Clothing, Food, School, Transport, and Other. This makes invalid user input of category impossible.
![drop-down menu](https://github.com/seng350/seng350f19-project-2-1/blob/master/docs/M4/Category.png)
- The cost can not be less than zero and can only include two desimals. If the user attempts to input an invalid cost and tries to submit, the purchase is not submitted and a pop up statement informs them of the requirement they are failing. 
![less than zero](https://github.com/seng350/seng350f19-project-2-1/blob/master/docs/M4/negative%20cost.jpg)
![two desimals](https://github.com/seng350/seng350f19-project-2-1/blob/master/docs/M4/less%20then%20cents.jpg)
- The calendar is a built in selection that can not accept invalid dates. It is possible to input a date down to 0001-01-01 which is unrealistic but not invalid. 
![built in selection](https://github.com/seng350/seng350f19-project-2-1/blob/master/docs/M4/calender.jpg)
- The description is limited to 15 characters including numbers, letters, and special characters. While special characters and numbers might not seem like good descriptions, the user is free to use them. The field will not accept any more characters after the limit is reached.


This use of data validation is sufficient for our users purchases and for the updating of purchases. Category is limited to choices rather than an input field for text, cost is limited to positive numbers with up to two decimals, date is limited by the selection from a calendar, and the description is only limited by length. The html form will not accept submissions that fail these criteria and so no database entry can have invalid data.
